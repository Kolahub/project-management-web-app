...
function Tasks() {
  const { project, handleDeleteTask } = useContext(ProjectsContext)
        ...
        <ul className="px-4  mt-8 rounded-md bg-stone-100">
          {arr.map((task, i) => (
            <li key={task.id} className={i !== arr.length - 1 ? 'flex justify-between border-b-2 border-stone-400 py-3' : 'flex justify-between py-3'}>
              <span>{task.text}</span>
              <button className="text-stone-700  hover:text-red-500" onClick={() => handleDeleteTask(project.id, task.id)}>
